<h1 align="center">Hi 👋, I'm Ajay Palvai</h1>
<h3 align="center">AI Engineer | Generative AI Developer | Full Stack AI Builder</h3>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com/?lines=AI+Engineer;LLM+Application+Developer;Generative+AI+Specialist;FastAPI+Developer;RAG+Systems+Builder;Open+Source+Contributor&center=true&width=800&height=45">
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Ajaypalvai07&label=Profile%20Views&color=0e75b6&style=flat" />
</p>

---

# 💫 About Me

🚀 AI Engineer with 3+ years of experience building scalable AI systems, intelligent automation platforms, and Generative AI applications.

🧠 Specialized in:
- Generative AI
- LLM Applications
- AI Agents
- RAG Pipelines
- Conversational AI
- Machine Learning
- Backend AI Systems

⚡ Currently building:
- AI Interview Platforms
- Recruitment AI Systems
- WhatsApp AI Agents
- Multi-Agent Architectures
- Voice AI Applications

🌱 Exploring:
- Advanced Agentic AI
- Multimodal AI
- MCP
- LangGraph
- Autonomous AI Workflows

💬 Ask me about:
`Python` `FastAPI` `LLMs` `RAG` `LangChain` `CrewAI` `AI Agents` `GenAI`

---

# ⚡ Current Focus

```yaml
Building:
  - AI Interview Platforms
  - AI Recruitment Systems
  - WhatsApp AI Agents
  - Multi-Agent AI Workflows
  - Autonomous AI Systems

Exploring:
  - Agentic AI
  - Multimodal AI
  - LangGraph
  - MCP Servers
  - AI Infrastructure
```

---

# 🌐 Connect With Me

<div align="center">

<a href="https://linkedin.com/in/ajay-palvai-384750210/" target="_blank">
<img src="https://skillicons.dev/icons?i=linkedin" height="55" />
</a>

<a href="https://github.com/Ajaypalvai07" target="_blank">
<img src="https://skillicons.dev/icons?i=github" height="55" />
</a>

<a href="mailto:YOUR_EMAIL">
<img src="https://img.icons8.com/color/96/gmail-new.png" height="55"/>
</a>

</div>

---

# 💻 Tech Stack

<div align="center">

<img src="https://skillicons.dev/icons?i=python,fastapi,react,nextjs,nodejs,js,ts,mongodb,mysql,postgres,docker,redis,azure,git,github,vscode" />

</div>

---

# 🤖 AI / GenAI Stack

<div align="center">

![OpenAI](https://img.shields.io/badge/OpenAI-GPT-black?style=for-the-badge&logo=openai)
![LangChain](https://img.shields.io/badge/LangChain-AI-green?style=for-the-badge)
![CrewAI](https://img.shields.io/badge/CrewAI-Agents-orange?style=for-the-badge)
![RAG](https://img.shields.io/badge/RAG-Pipeline-blue?style=for-the-badge)
![MCP](https://img.shields.io/badge/MCP-Agentic-red?style=for-the-badge)
![n8n](https://img.shields.io/badge/n8n-Automation-purple?style=for-the-badge)

</div>

---

# 📊 GitHub Stats

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=Ajaypalvai07&show_icons=true&theme=tokyonight&hide_border=true&border_radius=20"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ajaypalvai07&layout=compact&theme=tokyonight&hide_border=true&border_radius=20"/>

</div>

---

# 🔥 GitHub Streak

<div align="center">

<img src="https://streak-stats.demolab.com?user=Ajaypalvai07&theme=tokyonight&hide_border=true&border_radius=20" />

</div>

---

# 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=Ajaypalvai07&theme=tokyonight&no-frame=true&row=1&column=7" />

</div>

---

# 📈 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Ajaypalvai07&theme=tokyo-night&hide_border=true" />

</div>

---

# ⚡ Fun Fact

<div align="center">

```diff
+ I build AI systems that automate real-world business workflows 🚀
```

</div>

---

<div align="center">

## ⭐ "Building The Future With AI" ⭐

</div>

---

# 🌐 Connect With Me

<p align="left">
<a href="https://www.linkedin.com/in/ajay-palvai-384750210/" target="_blank">
<img align="center" src="https://skillicons.dev/icons?i=linkedin" height="40" />
</a>

<a href="https://github.com/Ajaypalvai07" target="_blank">
<img align="center" src="https://skillicons.dev/icons?i=github" height="40" />
</a>

<a href="mailto:ajay@spearsoftech.com">
<img align="center" src="https://img.icons8.com/color/48/gmail-new.png" height="40" />
</a>
</p>

---

# 💻 Tech Stack

## 🚀 Languages & Frameworks

<p align="left">
<img src="https://skillicons.dev/icons?i=python,js,ts,react,nextjs,nodejs,fastapi,flask,mongodb,mysql,postgres,docker,redis,azure,git,github" />
</p>

---

## 🤖 AI / GenAI Stack

<p align="left">

![LangChain](https://img.shields.io/badge/LangChain-000000?style=for-the-badge)
![CrewAI](https://img.shields.io/badge/CrewAI-FF6B00?style=for-the-badge)
![RAG](https://img.shields.io/badge/RAG-Pipeline-blue?style=for-the-badge)
![LLMs](https://img.shields.io/badge/LLMs-AI-green?style=for-the-badge)
![OpenAI](https://img.shields.io/badge/OpenAI-GPT-black?style=for-the-badge)
![Gemini](https://img.shields.io/badge/Gemini-AI-blueviolet?style=for-the-badge)
![MCP](https://img.shields.io/badge/MCP-Agentic-red?style=for-the-badge)
![n8n](https://img.shields.io/badge/n8n-Automation-orange?style=for-the-badge)

</p>

---

# 🏆 Featured AI Projects

## 🎯 AutoHire Prep
AI-powered career preparation platform with:
- AI Mock Interviews
- STT/TTS Voice Interviews
- Adaptive Learning
- Coding Practice
- Analytics Dashboards

### Stack:
`Python` `FastAPI` `React.js` `LLMs` `STT` `TTS`

---

## 🤖 AutoHire AI
AI recruitment automation platform featuring:
- Resume Screening
- AI Interview System
- Multi-turn Conversations
- Intent Routing
- Recruiter Analytics

### Stack:
`FastAPI` `LangChain` `CrewAI` `LLMs` `Machine Learning`

---

## 📈 LeadSpear.ai
AI-powered marketing automation and lead generation platform.

### Features:
- Lead Scoring
- Campaign Analytics
- Social Media Automation
- Personalized Outreach

### Stack:
`Python` `React.js` `AI` `Marketing Automation`

---

## 🧠 End-to-End RAG LLM
Advanced Retrieval-Augmented Generation system using:
- LLaMA2
- LlamaIndex
- Vector Search
- Semantic Retrieval

### Stack:
`RAG` `LLaMA2` `LlamaIndex` `Embeddings`

---

## 💬 EasySell AI
AI-powered WhatsApp & Instagram sales automation platform.

### Features:
- AI Agents
- WhatsApp Automation
- Payment Integration
- Order Tracking
- Multi-client Architecture

### Stack:
`FastAPI` `LangChain` `WhatsApp API` `Redis` `PostgreSQL`

---

## 📚 Second Brain Telegram Bot
Telegram-based personal knowledge assistant with vector search.

### Features:
- Semantic Search
- Vector DB
- Conversational AI
- Multimedia Storage

### Stack:
`Telegram Bot API` `LangChain` `Pinecone` `FAISS`

---

# 📊 GitHub Stats

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=Ajaypalvai07&show_icons=true&theme=tokyonight" height="180em"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Ajaypalvai07&layout=compact&theme=tokyonight" height="180em"/>
</p>

---

# 🔥 GitHub Streak

<p align="center">
<img src="https://streak-stats.demolab.com/?user=Ajaypalvai07&theme=tokyonight" />
</p>

---

# 🏆 GitHub Trophies

<p align="center">
<img src="https://github-profile-trophy.vercel.app/?username=Ajaypalvai07&theme=tokyonight&row=1&column=7" />
</p>

---

# 📈 Contribution Graph

<p align="center">
<img src="https://github-readme-activity-graph.vercel.app/graph?username=Ajaypalvai07&theme=tokyo-night" />
</p>

---

# 🧠 Currently Learning

- Agentic AI Systems
- Multi-Agent Orchestration
- LangGraph
- Autonomous Workflows
- Multimodal AI
- AI Infrastructure

---

# 🎯 Soft Skills

✅ Problem Solving  
✅ Analytical Thinking  
✅ Team Collaboration  
✅ Decision Making  
✅ Communication Skills  
✅ Time Management

---

# ⚡ Fun Fact

💡 I love building AI systems that solve real-world business problems and automate complex workflows.

🚀 Turning ideas into intelligent products with AI.

---

<h3 align="center">⭐ Let's Build AI That Matters ⭐</h3>
